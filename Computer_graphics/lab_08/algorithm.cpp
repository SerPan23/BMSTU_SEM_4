...
//     return (num > 0) ? 1 : -1;
// }
// int check_convex(clip_t &clip)
// {
//     size_t size = clip.points.size();
//     if (size < 3)
//         return false;

//     Point v1 = get_vector(line_t{clip.points[0], clip.points[1]});
//     Point v2 = get_vector(line_t{clip.points[1], clip.points[2]});

//     int s = sign(vector_product(v1, v2));


//     for (int i = 0; i < clip.points.size(); i++)
//     {

//         Point vi = get_vector(line_t{clip.points[i-2], clip.points[i-1]});
//         Point vj = get_vector(line_t{clip.points[i-1], clip.points[i]});

//         if (s * vector_product(vi, vj) < 0)
//             return false;
//     }

//     // if (s < 0)
//     //     std::reverse(clip.points.begin(), clip.points.end());
...
//     for (int i = 2; i < clip.points.size(); i++)
//     {
//         Point normal = get_normal(clip.points[i - 2], clip.points[i - 1], clip.points[i]);

//         Point w(src.start.x() - clip.points[i - 2].x(), src.start.y() - clip.points[i - 2].y());

//         int d_scalar = scalar_mul(D, normal);
//         int w_scalar = scalar_mul(w, normal);
...
//     res_line.start = Point(round(src.start.x() + D.x() * t_beg),
//                            round(src.start.y() + D.y() * t_beg));

//     res_line.end = Point(round(src.start.x() + D.x() * t_end),
//                            round(src.start.y() + D.y() * t_end));
