...
void Drawer::draw_clip(clip_t &clip, QColor& color)
{
    // for (int i = 1; i < clip.points.size(); i++)
    //     draw_line(clip.points[i - 1], clip.points[i], color);


    // if (clip.is_closed)
    //     draw_line(clip.points[clip.points.size() - 1], clip.points[0], color);


    int size = clip.lines.size();

    if (!clip.is_closed)
        --size;

    for (int i = 0; i < size; i++)
        draw_line(clip.lines[i].start, clip.lines[i].end, color);
